...
ARTICLES_TABLE = os.getenv("SUPABASE_ARTICLES_TABLE", "articles").strip() or "articles"
...
class ArticleParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if self._skip_from is None:
            marker = (data.get("class", "") + " " + data.get("id", "")).strip()
            if tag in self._HARD_SKIP or (marker and self._BAD_CLASS.search(marker)):
...
def call_groq(api_key: str, model: str, prompt: str, max_tokens: int = 2600) -> dict[str, Any]:
...
def _article_row(item: ArticleItem) -> dict[str, Any]:
    row = {
        "key": item.key.strip(),
        "title": item.title,
        "url": item.url,
        "host": item.host,
        "paragraphs": [str(p) for p in item.paragraphs[:200]],
    }
    if item.createdAt:
        row["created_at"] = item.createdAt
    return row
...
def _article_item(row: dict[str, Any]) -> dict[str, Any]:
...
def api_articles_list(x_app_code: str | None = Header(default=None)) -> dict[str, Any]:
    check_access(x_app_code)
    _require_sync()
    try:
        rows = supabase_call("GET", ARTICLES_TABLE, params={"select": "*", "order": "created_at.desc", "limit": "60"})
    except HTTPError as exc:
        raise HTTPException(status_code=502, detail=http_error_message(exc)) from exc
    except (URLError, TimeoutError, socket.timeout) as exc:
        raise HTTPException(status_code=502, detail="雲端文章紀錄暫時無法連線") from exc
    items = [_article_item(row) for row in rows] if isinstance(rows, list) else []
    return {"items": items}


@app.post("/api/articles")
def api_articles_upsert(payload: ArticleItem, x_app_code: str | None = Header(default=None)) -> dict[str, Any]:
    check_access(x_app_code)
    _require_sync()
    try:
        supabase_call("POST", ARTICLES_TABLE, payload=[_article_row(payload)], prefer="resolution=merge-duplicates,return=minimal")
    except HTTPError as exc:
        raise HTTPException(status_code=502, detail=http_error_message(exc)) from exc
    except (URLError, TimeoutError, socket.timeout) as exc:
        raise HTTPException(status_code=502, detail="文章紀錄同步失敗") from exc
    return {"ok": True}


@app.delete("/api/articles")
def api_articles_delete(key: str, x_app_code: str | None = Header(default=None)) -> dict[str, Any]:
    check_access(x_app_code)
    _require_sync()
    target = (key or "").strip()
    if not target:
        raise HTTPException(status_code=400, detail="缺少要移除的文章")
    try:
        supabase_call("DELETE", ARTICLES_TABLE, params={"key": f"eq.{target}"}, prefer="return=minimal")
    except HTTPError as exc:
        raise HTTPException(status_code=502, detail=http_error_message(exc)) from exc
    except (URLError, TimeoutError, socket.timeout) as exc:
        raise HTTPException(status_code=502, detail="文章紀錄移除失敗") from exc
    return {"ok": True}


def _groq_translate_batch(api_key: str, paragraphs: list[str], target_language: str) -> list[str]:
    model = os.getenv("GROQ_MODEL", "llama-3.3-70b-versatile").strip() or "llama-3.3-70b-versatile"
    if not re.fullmatch(r"[A-Za-z0-9._/-]+", model):
        model = "llama-3.3-70b-versatile"
    numbered = "\n".join(f"{i + 1}. {p}" for i, p in enumerate(paragraphs))
    prompt = (
        f"把下列英文段落翻成{target_language}，逐段對應、翻譯自然。"
        '只輸出一個 JSON 物件，格式：{"translations": ["譯文1", "譯文2"]}，'
        "陣列數量與段落數完全相同、順序一致，不要多餘文字。\n\n段落：\n" + numbered
    )
    response = call_groq(api_key, model, prompt, max_tokens=4000)
    text = groq_output_text(response).strip()
    text = re.sub(r"^```(?:json)?\s*|\s*```$", "", text, flags=re.I | re.S)
    try:
        parsed = json.loads(text)
...
def api_translate(payload: TranslateRequest, x_app_code: str | None = Header(default=None)) -> dict[str, Any]:
    check_access(x_app_code)
    groq_key = os.getenv("GROQ_API_KEY", "").strip()
    if not groq_key:
        raise HTTPException(status_code=503, detail="整篇翻譯需要 GROQ_API_KEY，請在 Vercel 設定後再試。")
    target = payload.targetLanguage if payload.targetLanguage in {"繁體中文", "日文", "英文"} else "繁體中文"
    paragraphs = [str(p) for p in payload.paragraphs if isinstance(p, str) and p.strip()][:40]
    if not paragraphs:
        return {"translations": []}
    try:
        translations = _groq_translate_batch(groq_key, paragraphs, target)
    except HTTPError as exc:
        message = http_error_message(exc)
        if exc.code == 429:
            message = "Groq 免費額度暫時用完或請求太頻繁，稍後再試。"
        elif exc.code in {401, 403}:
            message = "Groq API Key 無效或沒有權限。"
        raise HTTPException(status_code=502, detail=message) from exc
    except (URLError, TimeoutError, socket.timeout) as exc:
        raise HTTPException(status_code=502, detail="翻譯服務暫時無法連線，請稍後再試") from exc
